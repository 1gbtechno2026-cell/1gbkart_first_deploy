...
export default function FlashSale({ onQuickView }) {
  ...
  return (
    <section className="rounded-3xl overflow-hidden bg-gradient-to-br from-[#0d2f3f] via-[#0e4d63] to-[#2D9DBB] p-6 lg:p-8">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-white/50">Limited quantity — hurry!</p>
          </div>
          <CountdownTimer endMs={END} />
        </div>
        <div className="flex items-center gap-3">
          <span className="text-xs font-semibold text-white/80 cursor-pointer hover:text-white transition-colors whitespace-nowrap">View ALL →</span>
        ...
        {flashSaleProducts.map((p, i) => {
          ...
          return (
            <motion.div
              ...
              className="flex-shrink-0 w-40 sm:w-52 bg-white/8 backdrop-blur-sm border border-white/10 rounded-2xl overflow-hidden hover:bg-white/15 transition-all group cursor-pointer"
            ...
            >
              <div className="relative aspect-square overflow-hidden bg-white/5">
                ...
                {p.isNew && (
                  <span className="absolute top-2 right-2 bg-white text-[#2D9DBB] text-[10px] font-black px-1.5 py-0.5 rounded-lg">NEW</span>
                )}
              </div>
              <div className="p-2 sm:p-3">
                <p className="text-[10px] text-cyan-300 font-bold uppercase">{p.brand}</p>
                ...
                <div className="flex items-baseline gap-1.5 mt-2">
                  ...
                  <span className="text-[10px] text-white/40 line-through">₹{p.mrp.toLocaleString()}</span>
                </div>
                ...
                <div className="mt-2 mb-2">
                  <div className="flex justify-between text-[9px] text-white/50 mb-1">
                    <span>Sold {sold}%</span>
                    <span className="text-red-300 font-semibold">Only {p.stock} left</span>
                  </div>
                  <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
                    <motion.div
                      ...
                      className="h-full rounded-full bg-gradient-to-r from-white/80 to-red-400"
                    />
                  </div>
                </div>
                ...
                <button
                  ...
                  className="w-full py-2 bg-white text-[#2D9DBB] text-[11px] font-black rounded-xl hover:bg-white/90 transition-colors"
                >
                  Add to Cart
                </button>
              </div>
