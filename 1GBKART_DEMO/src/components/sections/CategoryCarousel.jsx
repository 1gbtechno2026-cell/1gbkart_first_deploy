    ...
    <div className="relative">
      ...
      >
        {categories.map((cat, i) => (
          ...
          >
            <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${cat.gradient} flex items-center justify-center text-2xl shadow-sm group-hover:shadow-md transition-all duration-200`}>
              {cat.icon}
            </div>
            <span className="text-xs font-semibold text-slate-700 group-hover:text-[#2D9DBB] transition-colors whitespace-nowrap">
              {cat.name}
            </span>
          </motion.button>
        ))}
